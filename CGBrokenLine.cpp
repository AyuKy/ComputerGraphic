...
// 将实现序列化宏中的类名修改为 CGBrokenLine
IMPLEMENT_SERIAL(CGBrokenLine, CGGeometry, 1)

CGBrokenLine::CGBrokenLine()
	: mClosed(false)
{
}

CGBrokenLine::CGBrokenLine(const std::vector<glm::dvec3>& points)
	: mPoints(points), mClosed(false)
{
}

CGBrokenLine::CGBrokenLine(const std::vector<glm::dvec3>& points, bool closed)
...
void CGBrokenLine::Serialize(CArchive& ar)
{
...
// 渲染
bool CGBrokenLine::Render(CGRenderContext* pRC, CGCamera* pCamera)
{
...
void CGBrokenLine::AddPoint(const glm::dvec3& point)
{
	mPoints.push_back(point);
}

void CGBrokenLine::Translate(float tx, float ty) {
	// 遍历所有点并应用平移
...
void CGBrokenLine::Rotate(double angle, double cx, double cy)
{
	// 如果点列表为空，直接返回
	if (mPoints.empty()) {
		return;
...
void CGBrokenLine::Scale(double sx, double sy, double cx, double cy)
{
	//创建平移矩阵（将中心点移动到原点）
