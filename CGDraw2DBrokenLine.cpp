...
#include "CG2022112465杨奎Doc.h" // 包含View之前要包含Doc
#include "CG2022112465杨奎View.h" // 用户交互绘制，并将图形对象通过Doc添加到场景
#include <gl/GL.h> // 包含 OpenGL 头文件


void CGDraw2DBrokenLine::draw2dline(glm::dvec3& s, glm::dvec3& e)
{
    glBegin(GL_LINES);
    glVertex2d(s.x, s.y);
    glVertex2d(e.x, e.y);
...
CGDraw2DBrokenLine::CGDraw2DBrokenLine(GLFWwindow* window)
    : UIEventHandler(window), mDrawing(false)
{
}

CGDraw2DBrokenLine::~CGDraw2DBrokenLine()
{
}

EventType CGDraw2DBrokenLine::GetType() //命令类型
{
    //UIEventHandler.h中定义，要根据不同的命令对象进行修改，此处是绘制折线
    return EventType::Draw2DLineStrip;
}
int CGDraw2DBrokenLine::OnMouseButton(GLFWwindow* window, int button, int action, int mods)
{
    if (window == nullptr)
        return -1;
    ...
    else if (button == GLFW_MOUSE_BUTTON_RIGHT && action == GLFW_PRESS)
    {
        if (mDrawing)
        {
            ...
            if (mPoints.size() >= 2) // 点数足够，创建折线
            {
                ...
                view->AddRenderable(std::make_shared<CGBrokenLine>(wcsPoints, false));
                view->Invalidate();
                view->UpdateWindow();
            }
            else
...
int CGDraw2DBrokenLine::OnCursorPos(GLFWwindow* window, double xpos, double ypos)
{
    if (window == nullptr)
        return -1;
    CCG2022112465杨奎View* view = (CCG2022112465杨奎View*)glfwGetWindowUserPointer(window);
    ...
    if (mDrawing)
    {
        ...
        glEnable(GL_COLOR_LOGIC_OP);
        glLogicOp(GL_XOR);
        glColor3f(0.0f, 0.0f, 1.0f); // 蓝色橡皮筋线
        if (mRubberBandMoveCount == 0)
        {
            // 第一次移动：直接绘制新线
            draw2dline(startWCS, currentWCS);
...
int CGDraw2DBrokenLine::Cancel(GLFWwindow* window)
{
    if (window == nullptr)
        return -1;
    CCG2022112465杨奎View* view = (CCG2022112465杨奎View*)glfwGetWindowUserPointer(window);
